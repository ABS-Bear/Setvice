...
async function submitLead(token,b){await install(token);const ph=phone(b.phone);if(ph.replace(/\D/g,'').length<5)return{ok:false,status:400,error:'Phone is required'};const kind=b.kind==='parts'?'parts':b.kind==='stationary-service'?'stationary-service':'service',parts=kind==='parts',stationary=kind==='stationary-service',label=parts?'ЗАПЧАСТИ':stationary?'СТАЦИОНАРНЫЙ СЕРВИС':'СЕРВИС',icon=parts?'🧩':stationary?'🏗':'🛠',aa=Array.isArray(b.attachments)?b.attachments.slice(0,MAX_ATTACH):[];const lines=[`${icon} <b>НОВАЯ ЗАЯВКА · ${label}</b>`,`<i>ABService · ${now()}</i>`,'',b.name?`👤 <b>Клиент:</b> ${esc(b.name)}`:null,`📞 <b>Телефон:</b> ${esc(ph)}`,b.machine?`🚜 <b>Техника:</b> ${esc(b.machine)}`:null,!parts&&b.location?`📍 <b>Локация:</b> ${esc(b.location)}`:null,!parts&&b.issue?`⚠️ <b>Проблема:</b> ${esc(b.issue)}`:null,parts&&b.mode?`🔧 <b>Формат:</b> ${esc(b.mode)}`:null,parts&&b.article?`🏷 <b>Артикул:</b> ${esc(b.article)}`:null,parts&&b.part?`📦 <b>Запчасть:</b> ${esc(b.part)}`:null,aa.length?`📷 <b>Вложений:</b> ${aa.length}`:null,'',`🌐 <b>Источник:</b> ${parts?'ABService · Запчасти':stationary?'ABService · Стационарный сервис':'ABService · Сервис'}`].filter(x=>x!==null);const sent=await tg(token,'sendMessage',{chat_id:CHAT,text:`${lines.join('\n')}${SEP}🔵 <b>Статус:</b> НОВАЯ`,parse_mode:'HTML',disable_web_page_preview:true,reply_markup:keys('new')});if(!sent.ok)return{ok:false,status:502,error:sent.data?.description||'Telegram failed'};const id=sent.data.result.message_id,c=await getCrm(token);if(c.ok){const lead={i:id,k:parts?'p':'s',st:'new',u:'',o:'',q:String(b.name||b.machine||b.part||ph).replace(/\s+/g,' ').trim().slice(0,28)};await saveCrm(token,c.id,record(c.state,{type:'new',kind,lead}))}let sentFiles=0,errs=[];for(let i=0;i<aa.length;i++){const r=await attachment(token,aa[i],`📎 ${label} · вложение ${i+1}/${aa.length} · ${ph}`);if(r.ok)sentFiles++;else errs.push(r.error||'attachment failed')}let bitrix24={ok:true,skipped:true,reason:'not configured'};try{bitrix24=await sendLeadToBitrix24(b,{telegramMessageId:id,normalizedPhone:ph,kind})}catch(err){console.error('bitrix24 adapter',err);bitrix24={ok:false,skipped:false,error:String(err?.message||err)}}return{ok:true,status:200,attachmentsRequested:aa.length,attachmentsSent:sentFiles,attachmentErrors:errs,bitrix24}}
...
export async function POST(req){const o=req.headers.get('origin')||'';if(o&&!isAllowedFrontendOrigin(o))return json(req,{ok:false,error:'Origin not allowed'},403);const token=process.env.TELEGRAM_BOT_TOKEN;if(!token||!CHAT||!INTERNAL)return json(req,{ok:false,error:'telegram config missing'},503);try{const b=await req.json();if(isTelegramWebhookUpdate(b)){const auth=verifyTelegramWebhookSecret(req);if(!auth.ok)return json(req,{ok:false,error:'unauthorized'},auth.status);if(b.callback_query)return json(req,await callback(token,b.callback_query));if(b.message)return json(req,await command(token,b.message));return json(req,{ok:true})}const r=await submitLead(token,b);return json(req,r,r.status||200)}catch(e){console.error(e);return json(req,{ok:false,error:'request failed'},500)}}

...
This v1.0.0 snapshot is prepared **locally**. It is not pushed, not uploaded to a customer GitHub repository, and does not change Vercel or Telegram.
...
- customer GitHub repository access (repository not created / not uploaded in this release);
...
- **CMS production authentication is not closed.** Marketers must not treat `/admin/` as a live production CMS.
- Customer GitHub repository, `siteUrl` / Astro `base`, Decap `backend.repo`, and Pages settings still match the current placeholder GitHub Pages path until remapped.
...
1. Customer-owned GitHub repository exists and this release is uploaded there (separate approval).
2. `public/admin/config.yml` `backend.repo` and `branch` match that repository.
3. GitHub OAuth app or Decap-compatible auth gateway is configured for the production admin URL.
...
6. Confirm admin remains noindex / robots-disallowed after the new base path.
